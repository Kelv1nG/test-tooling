...
func TestNormalizeTransferPage(t *testing.T) {
	tests := []struct {
		name  string
		value string
		want  int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := normalizeTransferPage(tt.value); got != tt.want {
				t.Fatalf("normalizeTransferPage(%q) = %d, want %d", tt.value, got, tt.want)
			}
		})
	}
}
